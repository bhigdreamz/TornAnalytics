...
export default function ToolCategoryPage() {
  ...
  // Fetch Tool items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Tool'],
    queryFn: async () => {
      console.log('Fetching Tool items directly...');
      const response = await fetch('/api/bazaar/items/Tool');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Tool items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Tools">
      <Helmet>
        <title>Tools | Byte-Core Vault</title>
        <meta name="description" content="Browse Tool items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Tool items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Tool items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Tool Items Found</h3>
