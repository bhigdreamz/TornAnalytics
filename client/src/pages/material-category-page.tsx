...
export default function MaterialCategoryPage() {
  ...
  // Fetch Material items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Material'],
    queryFn: async () => {
      console.log('Fetching Material items directly...');
      const response = await fetch('/api/bazaar/items/Material');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Material items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Materials">
      <Helmet>
        <title>Materials | Byte-Core Vault</title>
        <meta name="description" content="Browse Material items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Material items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Material items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Material Items Found</h3>
