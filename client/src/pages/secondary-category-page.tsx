...
export default function SecondaryCategoryPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [sortField, setSortField] = useState<string>("market_value");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");

  // Fetch Secondary items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Secondary'],
    queryFn: async () => {
      console.log('Fetching Secondary weapon items directly...');
      const response = await fetch('/api/bazaar/items/Secondary');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Secondary weapon items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Secondary Weapons">
      <Helmet>
        <title>Secondary Weapons | Byte-Core Vault</title>
        <meta name="description" content="Browse Secondary weapons in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Secondary Weapons</h1>
        </div>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Secondary weapons...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Secondary weapons. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Secondary Weapons Found</h3>
