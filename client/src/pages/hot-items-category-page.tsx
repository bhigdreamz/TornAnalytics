...
export default function HotItemsCategoryPage() {
  ...
  const highlightItemId = urlParams.get('highlight');
  
  console.log(`Hot items page loaded with location: ${location}`);
  console.log(`Highlight parameter: ${highlightItemId}`);
  
  // Refs for scrolling to items
  const itemRefs = useRef<Record<number, HTMLDivElement | null>>({});

  // Fetch Hot items directly using the trending endpoint
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/trending'],
    queryFn: async () => {
      console.log('Fetching hot/trending items directly...');
      const response = await fetch('/api/bazaar/items/trending');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} trending items`);
      return data;
    }
  });
  ...
  useEffect(() => {
    // Check sessionStorage for highlight info (from TOP DEALS clicks)
    ...
    if (itemIdToHighlight && !isLoading && items.length > 0) {
      const id = Number(itemIdToHighlight);
      console.log(`Hot items page: Looking for item ID ${id} to highlight`);
      console.log(`Available items:`, items.map(item => item.id).slice(0, 5));
      
      // Clear the sessionStorage after using it
      if (useStoredId) {
        sessionStorage.removeItem('highlightItemId');
        sessionStorage.removeItem('highlightTimestamp');
        console.log(`Cleared sessionStorage for item ${id}`);
      }
      
      // Wait for DOM to be fully rendered
      setTimeout(() => {
        const element = itemRefs.current[id];
        console.log(`Found element for item ${id}:`, element ? 'YES' : 'NO');
        
        if (element) {
          console.log(`Scrolling to item ${id} and highlighting`);
          ...
          setTimeout(() => {
            ...
            console.log(`Added highlight-item class to item ${id}`);
            
            // Remove highlight after animation
            setTimeout(() => {
              element.classList.remove('highlight-item');
              console.log(`Removed highlight-item class from item ${id}`);
            }, 3500);
          }, 800);
        } else {
          console.log(`Element not found for item ${id}`);
        }
      }, 1000);
    }
  }, [highlightItemId, isLoading, items]);
  ...
  return (
    <MainLayout title="Hot Items">
      <Helmet>
        <title>Hot Items | Byte-Core Vault</title>
        <meta name="description" content="Browse hot trending items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Hot Items</h1>
        </div>
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading trending items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length > 0 && (
        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
          <p className="text-sm text-muted-foreground">
            Found {filteredAndSortedItems.length} trending items
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredAndSortedItems.map(item => (
            <Card 
              ...
              className="border border-border hover:border-primary/50 transition-colors flex flex-col"
            >
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Trending Items Found</h3>
