...
export default function AlcoholCategoryPage() {
  ...
  // Fetch Alcohol items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Alcohol'],
    queryFn: async () => {
      console.log('Fetching Alcohol items directly...');
      const response = await fetch('/api/bazaar/items/Alcohol');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Alcohol items`);
      return data;
    }
  });
  ...
  useEffect(() => {
    if (highlightItemId && !isLoading && items.length > 0) {
      const id = Number(highlightItemId);
      ...
      setTimeout(() => {
        ...
        if (element) {
          ...
          // Add highlight effect with a brief delay to ensure scroll completes first
          setTimeout(() => {
            ...
            }, 3500); // Match the duration of our animation (3 pulses at 1s each + buffer)
          }, 200);
        }
      }, 300);

      // Log to help with debugging
      console.log(`Attempting to scroll to item ID: ${id}`);
    }
  ...
  return (
    <MainLayout title="Alcohol">
      <Helmet>
        <title>Alcohol | Byte-Core Vault</title>
        <meta name="description" content="Browse Alcohol items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Alcohol items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Alcohol items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Alcohol Items Found</h3>
