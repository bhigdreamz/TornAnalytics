...
export default function AllItemsCategoryPage() {
  ...
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/all'],
    queryFn: async () => {
      console.log('Fetching all items directly...');
      const response = await fetch('/api/bazaar/items/all');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} total items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="All Items">
      <Helmet>
        <title>All Items | Byte-Core Vault</title>
        <meta name="description" content="Browse all items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
        ...
        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <button 
              ...
              className={`flex items-center ${sortField === 'type' ? 'text-primary font-medium' : 'text-foreground'}`}
            >
