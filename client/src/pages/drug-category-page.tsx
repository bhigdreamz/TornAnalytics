...
export default function DrugCategoryPage() {
  ...
  // Fetch Drug items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Drug'],
    queryFn: async () => {
      console.log('Fetching Drug items directly...');
      const response = await fetch('/api/bazaar/items/Drug');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Drug items`);
      return data;
    }
  });
  ...
  // Manual test function to highlight all Xanax items on the page
  const highlightXanaxItems = () => {
    console.log("Manually highlighting all Xanax items on page");
    
    // Find all Xanax items on the page
    items.forEach(item => {
      if (item.name === "Xanax") {
        console.log(`Found Xanax item with ID: ${item.id}`);
        
        // Get the element
        const element = itemRefs.current[item.id];
        if (element) {
          console.log(`Found Xanax element in DOM, adding highlight class`);
          ...
          setTimeout(() => {
            ...
            console.log(`Scrolled to Xanax element at Y position: ${element.offsetTop}`);
          }, 1000);
        } else {
          console.log(`Xanax element with ID ${item.id} not found in DOM`);
        }
      }
    });
  };
  ...
  useEffect(() => {
    ...
    console.log("URLParam highlightId:", highlightItemId);
    console.log("SessionStorage itemId:", storedItemId);
    console.log("Using itemId:", itemIdToHighlight);
    
    // Debug all available refs
    console.log("Current itemRefs keys:", Object.keys(itemRefs.current));
    
    if (itemIdToHighlight && items.length > 0) {
      const id = Number(itemIdToHighlight);
      console.log(`Looking for item ID: ${id} in Drug category`);
      ...
      if (matchingItem) {
        console.log(`Found matching item: ${matchingItem.name} (ID: ${id})`);
      } else {
        console.log(`No matching item found for ID: ${id}`);
      }
      
      // We'll use a new approach with a fixed delay
      setTimeout(() => {
        // Force a direct DOM query to find the item
        const itemCard = document.querySelector(`[data-item-id="${id}"]`);
        
        if (itemCard) {
          console.log(`Found item element by data-attribute, scrolling now`);
          ...
          if (element) {
            console.log(`Found element via ref, scrolling now`);
            ...
            console.log(`Could not find element for item ID: ${id}`);
          }
        }
      }, 800); // Wait for DOM to be fully rendered
    }
  ...
  return (
    <MainLayout title="Drugs">
      <Helmet>
        <title>Drugs | Byte-Core Vault</title>
        <meta name="description" content="Browse Drug items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Drug items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Drug items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Drug Items Found</h3>
