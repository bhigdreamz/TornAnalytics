...
export default function MeleeCategoryPage() {
  ...
  // Fetch Melee items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    ...
    queryFn: async () => {
      ...
      console.log(`Received ${data.length} Melee items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Melee Weapons">
      <Helmet>
        <title>Melee Weapons | Byte-Core Vault</title>
        <meta name="description" content="Browse Melee weapons in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Melee Weapons</h1>
        </div>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Melee weapons...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Melee weapons. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Melee Weapons Found</h3>
