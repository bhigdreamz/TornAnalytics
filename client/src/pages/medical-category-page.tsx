...
export default function MedicalCategoryPage() {
  ...
  // Fetch Medical items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Medical'],
    queryFn: async () => {
      console.log('Fetching Medical items directly...');
      const response = await fetch('/api/bazaar/items/Medical');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Medical items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Medical Items">
      <Helmet>
        <title>Medical Items | Byte-Core Vault</title>
        <meta name="description" content="Browse Medical items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Medical Items</h1>
        </div>
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Medical items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Medical Items Found</h3>
