...
export default function EnhancerCategoryPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [sortField, setSortField] = useState<string>("market_value");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");

  // Fetch Enhancer items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Enhancer'],
    queryFn: async () => {
      console.log('Fetching Enhancer items directly...');
      const response = await fetch('/api/bazaar/items/Enhancer');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Enhancer items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Enhancers">
      <Helmet>
        <title>Enhancers | Byte-Core Vault</title>
        <meta name="description" content="Browse Enhancer items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold">Enhancers</h1>
        </div>
        
        <div className="relative w-full md:w-64">
          <Input
            type="text"
            placeholder="Search enhancers..."
            value={searchQuery}
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Enhancer items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Enhancer items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Enhancer Items Found</h3>
