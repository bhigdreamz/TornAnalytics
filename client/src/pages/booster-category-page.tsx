...
export default function BoosterCategoryPage() {
  const [searchQuery, setSearchQuery] = useState("");
  const [sortField, setSortField] = useState<string>("market_value");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");

  // Fetch Booster items directly using the specific category name
  const { data: items = [], isLoading, isError } = useQuery<TornItem[]>({
    queryKey: ['/api/bazaar/items/Booster'],
    queryFn: async () => {
      console.log('Fetching Booster items directly...');
      const response = await fetch('/api/bazaar/items/Booster');
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
      console.log(`Received ${data.length} Booster items`);
      return data;
    }
  });
  ...
  return (
    <MainLayout title="Boosters">
      <Helmet>
        <title>Boosters | Byte-Core Vault</title>
        <meta name="description" content="Browse Booster items in Torn bazaars with Byte-Core Vault." />
      </Helmet>
      ...
      {isLoading && (
        <div className="flex justify-center items-center h-40">
          ...
          <span className="ml-2">Loading Booster items...</span>
        </div>
      )}
      ...
      {isError && (
        <div className="p-4 border border-red-300 bg-red-50 text-red-700 rounded-md">
          Error loading Booster items. Please try again later.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredAndSortedItems.length === 0 && (
        <div className="text-center py-10">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No Booster Items Found</h3>
